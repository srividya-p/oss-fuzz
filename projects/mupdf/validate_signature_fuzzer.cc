...
{
    if (size < 4)
        return 0; // skip very small inputs
    ...
    fz_try(ctx)
    {
        ...
        {
            fz_page *fzpage = fz_load_page(ctx, doc, i);
            ...
            pdf_page *page = (pdf_page *)fzpage;
            for (pdf_annot *annot = pdf_first_widget(ctx, page); annot != NULL; annot = pdf_next_widget(ctx, annot))
            {
                if (pdf_widget_type(ctx, annot) == PDF_WIDGET_TYPE_SIGNATURE)
                {
                    int is_signed;
                    is_signed = pdf_widget_is_signed(ctx, annot);
                    if (is_signed)
                    {
                        int valid_until;
                        valid_until = pdf_validate_signature(ctx, annot);
                    }
                }
            }
            fz_drop_page(ctx, fzpage);
